...
class TuneAVideoPipeline(DiffusionPipeline):
    ...
    def decode_latents(self, latents):
        ...
        for i in range(max(latents.shape[0]//bs, 1)):
            ...
            video = video.cpu().float().detach().numpy()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Encode input prompt
        ######## TEXTUAL INVERSION ########
        # if 'embedding_manager' in kwargs.keys():
        #     text_embeddings = kwargs['embedding_manager'].inference(
        ...
        latents = self.prepare_latents(
            ...
            height,
            width,
            text_embeddings.dtype if not isinstance(text_embeddings, list) else text_embeddings[0].dtype,
            device,
            generator,
            latents,
        )
            ...
            for i, t in enumerate(timesteps):
                ...
                if uncond_embeddings_pre is not None:
                    if isinstance(text_embeddings, list):
                        # use Prompt+
                        text_embeddings = [torch.cat([uncond_embeddings_pre[i], emb[1]], 0) for emb in text_embeddings]
                    else:
                ...
                if controller is not None:
                    ...
                    # from utils.ptp import show_cross_attention_per_frame
                    # show_cross_attention_per_frame(
                    #     attention_store=controller, res=-1, from_where=["up", "down"], tokenizer=self.tokenizer,
                    #     prompts=prompt,
                    #     video_len=video_length, select=1, select_words=["roaring"]  ##############
                    # )

                    if getattr(controller, 'zero_attn_probs', None):
